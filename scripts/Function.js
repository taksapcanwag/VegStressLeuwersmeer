...
var harmonizationRoy = function(oli) {
  var slopes = ee.Image.constant([0.9785, 0.9542, 0.9825, 1.0073, 1.0171, 0.9949]);        // RMA - create an image of slopes per band for L8 TO L7 regression line
  var itcp = ee.Image.constant([-0.0095, -0.0016, -0.0022, -0.0021, -0.0030, 0.0029]);     // RMA - create an image of y-intercepts per band for L8 TO L7 regression line
...
var getSRcollection = function(year, startDay, endDay, sensor, aoi, maskThese, exclude) {
  ...
  var maskOptions = ['cloud', 'shadow', 'snow', 'water', 'waterplus','nonforest']; 
...
var buildMosaic = function(year, startDay, endDay, aoi, dummyCollection, maskThese, exclude) {                                                                     
  exclude = (typeof exclude !== 'undefined') ?  exclude : {}; // default to not exclude any images
...
var calcIndex = function(img, index, flip){
  ...
  switch (index){
    case 'B1':
      indexImg = img.select(['B1']).float();
      break;
    case 'B2':
      indexImg = img.select(['B2']).float();
      break;
    case 'B3':
      indexImg = img.select(['B3']).float();
      break;
    case 'B4':
      indexImg = img.select(['B4']).multiply(indexFlip).float();
      break;
    case 'B5':
      indexImg = img.select(['B5']).float();
      break;
    case 'B7':
      indexImg = img.select(['B7']).float();
      break;
    case 'NBR':
...
var getFittedData = function(lt, startYear, endYear, index, ftvlist){
  ...
  if(ftvlist === true){
    ...
    search = 'ftv_'+ftvlist[0].toLowerCase()+'_fit'; 
...
var getSegmentDictionary = function(lt, options) {
  ...
  var midYear = (((startYear).add(endYear)).divide(2)).round() 
...
var getSegmentData = function(lt, index, delta, options){
  ...
  */
  ...
  if(delta.toLowerCase() == 'all'){
    ...
    return ee.Image.cat([
        startYear.add(1).arrayMask(changeTypeMask),
        endYear.arrayMask(changeTypeMask),
        startVal.arrayMask(changeTypeMask).multiply(flip),
        endVal.arrayMask(changeTypeMask).multiply(flip),
        mag.arrayMask(changeTypeMask).abs(),
        dur.arrayMask(changeTypeMask),
        rate.arrayMask(changeTypeMask).abs(),
        dsnr.arrayMask(changeTypeMask).abs(),
      ])
      .unmask(ee.Image(ee.Array([[-9999]])))
      .toArray(0); 
  }        
...
var stdDevStretch = function(img, aoi, nStdev){
  var per95 = img.reduceRegion({reducer:ee.Reducer.percentile([95]), geometry:aoi, scale:900, bestEffort:true, maxPixels:1e4})
              .toArray()
              .reduce(ee.Reducer.percentile([95]), [0]);
...
exports.yearPanel = function(){
  ...
  var startYearslider = ui.Slider({min:1985, max:2021, value:1985, step:1});
  startYearslider.style().set('stretch', 'horizontal');
  
  var endYearLabel = ui.Label('End Year:');
  var endYearslider = ui.Slider({min:1985, max:2021, value:2021, step:1});
...
exports.getFileName = function(fileNamePanel){     
...
exports.getChangeMap = function(lt, changeParams){
  ...
  switch (changeParams.sort.toLowerCase()){
    case 'greatest':
      sortByThis = changeParams.segInfo.arraySlice(0,4,5).multiply(-1); 
      break;
    case 'least':
      sortByThis = changeParams.segInfo.arraySlice(0,4,5);
      break;
    case 'newest':
      sortByThis = changeParams.segInfo.arraySlice(0,0,1).multiply(-1); 
      break;
    case 'oldest':
    ...
    case 'fastest':
      ...
      break;
    case 'slowest':
      sortByThis = changeParams.segInfo.arraySlice(0,5,6).multiply(-1); 
      break;
  }
